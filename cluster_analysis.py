...
payments_df.loc[payments_df['payments'] > 3, 'payments'] = 4
...
# payments or not between low and medium or high
payments_df['paid'] = payments_df['payments'] > 0
fig = px.histogram(payments_df, 'paid', color='type', barmode='group', histnorm='probability density')
fig.update_yaxes(showgrid=True, gridwidth=0.1, griddash='dash', gridcolor='black',
                 showline=True, linewidth=1, linecolor='black')
fig.update_xaxes(showline=True, linewidth=1, linecolor='black')
fig.update_layout(paper_bgcolor='rgba(0, 0, 0, 0)', plot_bgcolor='rgba(0, 0, 0, 0)')
fig.show()
fig.write_html('paid-low-vs-medium-high.html')
fig.write_image('paid-low-vs-medium-high.svg')
...
judge_payments_df.loc[judge_payments_df['payments'] > 3, 'payments'] = 4
