...
'''
import copy
from pandas.core.util.numba_ import types
...
from utils import dijkstra_less_memory
...
def apply_trace(trace, petri_net, initial_marking, final_marking, parameters=None,
    ...
    """
    if parameters is None:
        parameters = copy.copy({PARAMETER_CONSTANT_ACTIVITY_KEY: DEFAULT_NAME_KEY})
    ...
    if ali is not None and best_worst_cost is not None:
        if not isinstance(ali, types.GeneratorType):
            ...
            # Add for multiple alignments purpose
            # breakpoint()
            alis = []
            for align in ali:
                ltrace_bwc = trace_cost_function_sum + best_worst_cost

                fitness_num = align['cost'] // align_utils.STD_MODEL_LOG_MOVE_COST
                fitness_den = ltrace_bwc // align_utils.STD_MODEL_LOG_MOVE_COST
                fitness = 1 - fitness_num / fitness_den if fitness_den > 0 else 0
                align["fitness"] = fitness
                # returning also the best worst cost, for log fitness computation
                align["bwc"] = ltrace_bwc
                alis.append(align)
            ali = copy.deepcopy(alis)
...
def apply_log(log, petri_net, initial_marking, final_marking, parameters=None, variant=DEFAULT_VARIANT):
    ...
    for trace in one_tr_per_var:
        ...
        alignments_trace = apply_trace(trace, petri_net, initial_marking, final_marking, parameters=copy.copy(parameters),
                                          variant=variant)
        for alignment in alignments_trace:
...
def __get_best_worst_cost(petri_net, initial_marking, final_marking, variant, parameters):
    parameters_best_worst = copy.copy(parameters)
